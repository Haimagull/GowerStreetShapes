"""
This script aims to evaluate DM halo shape distribution at a given redshift 
(to later study dependency on redshift variation). 
...
def run_control_par(run, control_path) :# reading control.par file from run
...
def particle_mass(time_slice, run) :
    ...
    dBoxSize, nGrid, dOmega0, h = run_control_par(run, control_path)
    # Calc.
...
for run in runs :
    ...
    for time_slice in times :
        plot_contour(time_slice, run=run)
